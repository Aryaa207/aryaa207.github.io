...
try {for(const route of routes){
 // The homepage has the strictest budget. Always collect three independent
 // navigations and use Lighthouse's representative run, rather than a cold-run outlier.
 const samples=[];
 for(let sample=0;sample<(route==='/'?3:1);sample++){
  ...
  if(!result)throw new Error('No Lighthouse result');samples.push(result.lhr);
  await writeFile(`.lighthouseci/${route.replaceAll('/','_')}-sample-${sample+1}.json`,JSON.stringify(result.lhr));
  console.log(JSON.stringify({route,sample:sample+1,performance:result.lhr.categories.performance.score,tbtMs:result.lhr.audits['total-blocking-time'].numericValue,speedIndexMs:result.lhr.audits['speed-index'].numericValue}));
 }
 const lhr=computeMedianRun(samples);
